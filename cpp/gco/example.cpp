...
void GridGraph_Individually(int width,int height,int num_pixels,int num_labels)
{
	...
	try{
		...
		printf("\nBefore optimization energy is %lld",(long long)gc->compute_energy());
		gc->expansion(2);// run expansion for 2 iterations. For swap use gc->swap(num_iterations);
		printf("\nAfter optimization energy is %lld",(long long)gc->compute_energy());
...
int main(int argc, char **argv)
{
	...
	printf("\n  Finished %lld (%lld) clock per sec %lld",(long long)(clock()/CLOCKS_PER_SEC),(long long)clock(),(long long)CLOCKS_PER_SEC);
